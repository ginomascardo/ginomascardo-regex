...
public class NFA {
	...
	protected void toDOT_(State p, StringBuilder buf, Set<Integer> visited) {
		/* TODO:
		if we have visited p before, return
		mark p as visited
		for each edge e of p:
			if epsilon edge, label is "&epsilon;"
			else label is e.label
			append "  %d -> %d [label=\"%s\", fontsize=18];\n" to buf
				with appropriate p state number, target state number, and label
			recurse to toDOT_() with appropriate parameters
		*/
	}
	...
	protected void getStates_(State p, LinkedHashSet<State> visited) {
        ...
        mark p as visited
        for each edge e of p:
            recurse to getStates_() with appropriate parameters
        */
	}
